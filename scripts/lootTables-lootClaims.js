...
export async function addLootTable(tokens, lootTable) {
    ...
    if (game.betterTables) {
        ...
        for (let token of tokens) {
            //UseBetterTables(token, lootTable);

            let docs = await getDocsViaBetterTables(token, lootTable);
...
async function UseBetterTables (token, realTable) {
    ...
    for (const lootItem of drawnItems) {
        ...
        */
        ...
        // TODO: **UNLESS** It's an item with charges that _isn't_ destroy-on-empty.  That gets weird.
        //       See: our Alchemist's Flask that's implemented as an improvised weapon.
        if (existingItem) {
...
// Testing
async function getDocsViaBetterTables(token, realTable) {
    // Clone the table so we can do whatever without worry.
    // Including just drawing without replacement.
    let table = realTable.clone({}, {}, {temporary:true});
    //const newLoot = {};
